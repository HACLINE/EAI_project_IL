...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # print(root, episodes, image_transforms, delta_timestamps, tolerance_s, revision, force_cache_sync, download_videos, video_backend, batch_encoding_size)
        # exit()
        """
...
class Sam3LeRobotDataset(LeRobotDataset):
    def __init__(
        self,
        repo_id: str,
        root: str | Path | None = None,
        ...
        self.server_url = "http://localhost:8002/predict"
        self.sam_cache_dir = self.root / "sam_cache"
        self._last_loaded_ep_idx = -1
        self._last_loaded_ep_cache = None

        if not self.sam_cache_dir.exists():
            print(f"SAM cache not found at {self.sam_cache_dir}. Precomputing...")
            self._precompute_sam()
        else:
            print(f"Using existing SAM cache at {self.sam_cache_dir}")

    def _precompute_sam(self):
        self.sam_cache_dir.mkdir(parents=True, exist_ok=True)
        current_ep_idx = None
        ep_buffer = {}
        
        try:
            from tqdm import tqdm
            iterator = tqdm(range(len(self)), desc="Precomputing SAM")
        except ImportError:
            iterator = range(len(self))

        for idx in iterator:
            # Use super() to get the raw item without SAM data
            ...
            if ep_idx != current_ep_idx:
                self._save_buffer(current_ep_idx, ep_buffer)
                current_ep_idx = ep_idx
                ep_buffer = {}
            
            updates = self._compute_sam_updates(item)
            
            for k, v in updates.items():
                if k not in ep_buffer:
                    ep_buffer[k] = []
                ep_buffer[k].append(v.cpu())
        
        if current_ep_idx is not None:
            self._save_buffer(current_ep_idx, ep_buffer)

    def _save_buffer(self, ep_idx, buffer):
        data = {k: torch.stack(v) for k, v in buffer.items()}
        torch.save(data, self.sam_cache_dir / f"ep_{ep_idx}.pt")

    def _extract_segment_prompts(self, task):
        segment_prompts = []
        if 'red' in task or 'cubes' in task:
            segment_prompts.append("red block")
        if 'green' in task or 'cubes' in task:
            segment_prompts.append("green block")
        return segment_prompts

    def _sam_inference(self, image, prompt):
        # Serialize image
        buffer = io.BytesIO()
        ...
        try:
            ...
            masks = torch.load(io.BytesIO(base64.b64decode(result["masks"])))
            boxes = torch.load(io.BytesIO(base64.b64decode(result["boxes"])))
            scores = torch.load(io.BytesIO(base64.b64decode(result["scores"])))
            
            return masks, boxes, scores
        except Exception as e:
            print(f"Error calling SAM server: {e}")
            raise e

    def _compute_sam_updates(self, item):
        updates = {}
        segment_prompts = self._extract_segment_prompts(item['task'])

        image_keys = [key for key in item.keys() if key.startswith('observation.images.') and not key.endswith('pad')]
        image_keys = list(sorted(image_keys))
        print(image_keys)

        for prompt in segment_prompts:
            for image_key in image_keys:
                updates[f'{image_key}.{prompt.split()[0]}'] = []

        all_additional_states = []

        # Assuming all images have the same number of frames (batch size)
        num_frames = item[image_keys[0]].shape[0]

        for i in range(num_frames):
            single_addition_states = []
            for prompt in segment_prompts:
                prompt_states = []
                for image_key in image_keys:
                    image = item[image_key][i]
                    
                    print(image.shape)
                    masks, boxes, scores = self._sam_inference(image, prompt)
                    ...
                    if boxes.shape[0] >= 1:
                        ...
                        masks = torch.zeros(1, image.shape[1], image.shape[2]).to(torch.bool)
                    
                    updates[f'{image_key}.{prompt.split()[0]}'].append(masks.to(torch.float32))
                    
                    prompt_states.append(boxes)
                    prompt_states.append(scores)
                
                single_addition_states.append(torch.concatenate(prompt_states, dim=0))
            
            single_addition_states = torch.concatenate(single_addition_states, dim=0)
            all_additional_states.append(single_addition_states)

        all_additional_states = torch.stack(all_additional_states, dim=0)
        updates['observation.state'] = torch.concatenate([
        ...
        for prompt in segment_prompts:
            for image_key in image_keys:
                updates[f'{image_key}.{prompt.split()[0]}'] = torch.stack(
                    updates[f'{image_key}.{prompt.split()[0]}'], dim=0
                )
    ...
    def __getitem__(self, idx) -> dict:
        ...
        ep_idx = item['episode_index'].item()
        # frame_index in item is relative to episode, but we are caching by global index (idx) effectively?
        # No, we are caching by episode, and inside episode by frame_index.
        # But wait, if we iterate range(len(self)), we get items in order.
        # If we stack them in order, the index in the stack corresponds to the order they appeared in the dataset for that episode.
        # Assuming LeRobotDataset returns items in order of frame_index for each episode.
        # Yes, usually.
        
        frame_idx = item['frame_index'].item()
        
        if ep_idx != self._last_loaded_ep_idx:
            cache_path = self.sam_cache_dir / f"ep_{ep_idx}.pt"
            if cache_path.exists():
                self._last_loaded_ep_cache = torch.load(cache_path)
                self._last_loaded_ep_idx = ep_idx
            else:
                self._last_loaded_ep_cache = None

        if self._last_loaded_ep_cache:
            for k, v in self._last_loaded_ep_cache.items():
                # v is stacked tensor for the episode.
                # We assume frame_idx is the correct index into this stack.
                item[k] = v[frame_idx]
        else:
             updates = self._compute_sam_updates(item)
             item.update(updates)
        ...
        return item
        """
    observation.images.front: torch.Size([2, 3, 480, 640])
    observation.images.wrist: torch.Size([2, 3, 480, 640])
    action: torch.Size([16, 6])
    observation.state: torch.Size([2, 6])
    timestamp: torch.Size([])   
    frame_index: torch.Size([])
    episode_index: torch.Size([])
    index: torch.Size([])
    task_index: torch.Size([])          
    action_is_pad: torch.Size([16])
    observation.state_is_pad: torch.Size([2])
        """
